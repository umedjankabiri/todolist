...
test('status of specified task should be changed', () => {
    const endState = tasksReducer(initialState, changeTaskStatusAC({todolistID: todolistID2, taskID: '2', isDone: false}))

    expect(endState[todolistID1][1].isDone).toBe(true)
    expect(endState[todolistID2][1].isDone).toBe(false)
})
test('title of specified task should be changed', () => {
    const endState = tasksReducer(initialState, changeTaskTitleAC({todolistID: todolistID2, taskID: '2', title: "coffee"}))
